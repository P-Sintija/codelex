...
class FizzBuzz
{
    ...
    {
        $this->input = readline("Enter integer :");
    }

    function inputIntegers(string $input): array
    {
        $storage = [];
        for ($i = 1; $i <= $input; $i++) {
            $storage[] = $i;
    ...
    function makeArray(array $integers, string $input): array
    {
        $grid = [];
        for ($i = 0; $i < $input / self::rowLength; $i++) {
            $grid[$i] = array_slice($integers, ($i * self::rowLength), self::rowLength);
        }
        return $grid;
    }

    function reFizzBuzzIntegers(array $integers): array
    {
        for ($i = 0; $i < count($integers); $i++) {
            for ($j = 0; $j < count($integers[0]); $j++) {
                if ($integers[$i][$j] % 3 === 0 && $integers[$i][$j] % 5 === 0) {
                    $integers[$i][$j] = "FizzBuzz";
                } else if ($integers[$i][$j] % 3 === 0) {
                    $integers[$i][$j] = "Fizz";
                } else if ($integers[$i][$j] % 5 === 0) {
                    $integers[$i][$j] = "Buzz";
                }
            }
        }
